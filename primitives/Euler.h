...
struct HeunStep : public StepSolverBase<TBlas<RealT> >{
	static void call(unsigned int N,RealT t,RealT h, Vector &x, const Func &F,const History* history = 0){
		//x = x+ h/(real_t)2.*(F(t,x)+F(t+h,x+h*F(t,x)));
		typedef TBlas<RealT> Blas;
		typename StepSolverBase<TBlas<RealT> >::MyBlasVector ftx(N);
		F(t,x,ftx);					//ftx = F(t,x)

		typename StepSolverBase<TBlas<RealT> >::MyBlasVector y2(N);
		Blas::copy(N,x,y2);
		Blas::axpy(N,h,ftx,y2);		//x+h*F(t,x);  (y2= ftx*h + y2)

		typename StepSolverBase<TBlas<RealT> >::MyBlasVector ftx2(N);
